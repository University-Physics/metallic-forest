...
std::vector<int> vecinos(int Na, std::vector <int> &a)
{
  ...
  */
  std::vector<int> veci;
  if(Na+ancho<a.size()) veci.push_back(a[Na+ancho]); // Se refiere al vecino del sitio de abajo
  else veci.push_back(2);/*Si se quieren fronteras arriba y abajo periódicas descomentar esta condición y comentar la anterior:*/
  //else veci.push_back(3);  
  if(Na-ancho>=0) veci.push_back(a[Na-ancho]); // se refiere al vecino del sitio de arriba 
  else veci.push_back(2);/*Si se quieren fronteras arriba y abajo periódicas descomentar esta condición y comentar la anterior:*/
  //else veci.push_back(3);  
  if(Na+1<a.size() && (Na+1)%ancho!=0) veci.push_back(a[Na+1]); // Se refiere al vecino del sitio de la derecha
...
int casilla(int N)
{
  ...
  if(N==0) return ancho; // Número de sitios de casillas para ir al sitio de abajo
  else if(N==1) return (-ancho); // Número de sitios de casillas para ir al sitio de arriba
...
while(true)
    {
     ...
     if( Na+casilla(random)<a.size() && Na+casilla(random)>0 && b[random]!=2) {Na=Na+casilla(random);} /* La casilla a la que la partícula se movera debe cumplir unas condiciones, debe estar en el arreglo y no debe ser prohibida, es decir no se puede pasar del lado derecho del arreglo al izquierdo en un solo paso y viceversa.*/
      else {break;}// Si se quieren condiciones de frontera arriba y abajo comentar esta linea y descomentar la siguiente:
     /*else {
       if(casilla(random)==-1100 && b[random]==3) Na=a.size()-(1100-Na);
       else {
       if (casilla(random)==1100 && b[random]==3) Na=Na-1100*1099;
       else break;
       }
       }
     */
...
void print(std::vector <int> &a, std::string &b)
{
  ...
  for(int i=0;i<ancho;i++)
    {
      for(int ii=0;ii<largo;ii++){fout<<i<<" "<<ii<<" "<<a[i*ancho+ii]<<" "<<std::endl;}
